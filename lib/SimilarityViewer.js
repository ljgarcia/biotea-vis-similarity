...
SimilarityViewer.prototype.selectById = function(id, datum) {
    ...
    if (!datum) {
        datum = _.filter(viewer.nodes, function(node) {
    ...
    if (!datum) {
        ...
        d3.select(viewer.options.id).selectAll('.biotea_node circle, .biotea_query_node circle')
            .style('stroke', function() {
                return d3.select(this).style('fill');
            })
            .style('stroke-width', '1px');

        var elem = d3.select(viewer.options.id).select('#biotea_sim_node_' + id);
