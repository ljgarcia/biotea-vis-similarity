...
SimilarityViewer.prototype.display = function() {
    ...
    var node = viewer.svg.selectAll('.node')
        ...
        .attr('id', function(d) {
            return ('biotea_sim_node_' + d.relatedId);
        })
        ...
        .on('click', function(d) {
            viewer.selectById(d.relatedId, d);
        });
...
var createForceGraph = function(viewer) {
    viewer.data.relations = _.filter(viewer.data.relations, function(rel) {
...
SimilarityViewer.prototype.setOptions = function(options) {
    this.options.prefixId = options.prefixId;
    this.options.queryId = options.queryId;
    this.options.alternativePrefixId = options.alternativePrefixId;
    this.options.alternativeQueryId = options.alternativeQueryId;
    this.options.data = options.data;
    this.options.useAlternativeIds = options.useAlternativeIds;
...
SimilarityViewer.prototype.selectById = function(id, datum) {
    var viewer = this;

    if (!datum) {
        datum = _.find(viewer.nodes, function(node) {
            return node.relatedId === id;
        });
    }

    if (!datum) {
        viewer.dispatcher.notInData({type: 'similarity', id: id});
    } else {
        d3.select(viewer.options.el).selectAll('.biotea_node circle, .biotea_query_node circle')
            .style('stroke', function() {
                return d3.select(this).style('fill');
            })
            .style('stroke-width', '1px');

        var elem = d3.select(viewer.options.el).select('#biotea_sim_node_' + id);
        elem.selectAll('circle').style('stroke', 'black').style('stroke-width', '3px');

        viewer.dispatcher.selected({type: 'similarity', elem: elem, datum: datum});
    }
};
