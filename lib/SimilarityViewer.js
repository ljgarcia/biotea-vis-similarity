...
var SimilarityViewer = function(opts){
    ...
    viewer.display = function() {
        ...
        var force = d3.layout.force().charge(-200).linkDistance(50)
        ...
        force.nodes(nodes).links(viewer.data.links).start();
        ...
        force.on("tick", function() {
            ...
            node.attr("transform", function(d) {
                var left = d.x < node[0][0].__data__.x ? true : false;
                if (left) {
                    var textWidth = this.childNodes[1].getBBox().width;
                    var shiftX = -radius(d.score) - 15 - textWidth;
